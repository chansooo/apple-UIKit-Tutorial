...
final class ReminderListViewController: UICollectionViewController {
    ...
    override func viewDidLoad() {
        ...
        // collectionView의 layout을 listLayout에서 정의한 layout으로 지정
        let listLayout = listLayout()
        collectionView.collectionViewLayout = listLayout
        
        let cellRegistration = UICollectionView.CellRegistration(handler: cellRegistrationHandler)
        
        // UICollectionViewDiffableDataSource를 생성(?)
    ...
    /// 매개변수로 전달받은 ID의 미리알림의 상세보기 페이지로 이동하는 함수
    ...
    /// UICollectionView의 layout을 설정하는 함수
    /// - Returns: 아래 함수에서 설정한 내용의 UICollectionViewCompositionalLayout
    private func listLayout() -> UICollectionViewCompositionalLayout {
        var listConfiguration = UICollectionLayoutListConfiguration(appearance: .grouped)
        listConfiguration.headerMode = .supplementary
        listConfiguration.showsSeparators = false // Separators 비활성화
        listConfiguration.trailingSwipeActionsConfigurationProvider = makeSwipeActions
        listConfiguration.backgroundColor = .clear // 배경색 지우기
        return UICollectionViewCompositionalLayout.list(using: listConfiguration)
    }
    
    /// UICollectionViewCell을 스와이프 했을 때 삭제 액션을 추가하는 함수
    /// - Parameter indexPath: UICollectionViewCell의 indexPath
    /// - Returns: delete Action을 추가한 UISwipeActionsConfiguration
